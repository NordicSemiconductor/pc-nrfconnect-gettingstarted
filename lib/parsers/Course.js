...
export default class Course extends AbstractParser {
    ...
    constructor(json, searchPath) {
        super(json, 'Course', -1, [
            ...
            },
            {
                name: 'recipes',
                type: Array,
                processer: Course.extractRecipes(searchPath),
                promise: true,
            },
        ]);
    }
    ...
    loadRecipes() {
        return this.recipesPromise.then(recipes => {
    ...
    static loadFromFile(filename) {
        return readFile(filename, { encoding: 'utf8' })
            ...
            .catch(error => {
                throw new Error(
                    `File ${filename} failed to be parsed as JSON: ${error}`
                );
            })
        ...
        return recipes =>
            Promise.all(
                recipes
                    ...
                    .map(filename =>
                        searchPath ? path.join(searchPath, filename) : filename
                    )
                    .map((filepath, recipeId) =>
                        Recipe.loadFromFile(filepath, recipeId)
                    )
            );
    }
