...
export default class AbstractParser {
    constructor(json, type, id, properties) {
        ...
        if (this.enabled) {
            properties.forEach(property =>
                this.extractProperty(json, property)
            );
        }
    }
            ...
            do {
                ...
                if (match !== null) {
                    ...
                    if (variables[variable]) {
                        ...
                        logger.info(
                            `Variable ${variable} is set to default value: ${defaultValue}`
                        );
                    } else {
    ...
    extractProperty(json, property) {
        ...
        if (propertytype !== undefined) {
            if (!(json[name] instanceof propertytype)) {
                throw new Error(
                    `"${name}" field in JSON is not ${propertytype}.`
                );
            }
        }
    ...
    parseVariablesFrom(elements) {
        (elements || []).forEach(element => {
            Object.keys(element.variables).forEach(key => {
                this.variables[key] =
                    this.variables[key] || element.variables[key];
            });
        });
    }
    ...
    static processDescriptionArray(description, joiner = '\n\n') {
        const descriptionArray =
            description instanceof Array ? description : [description];
        ...
        const process = item => {
            ...
            if (item.type === 'commands') {
                const itemDescription = AbstractParser.processDescriptionArray(
                    item.description,
                    '\n'
                );
    ...
    serialize() {
        throw new Error(
            'This class should never be serialized. Implement the serialize function in the subclass.'
        );
    }
}
