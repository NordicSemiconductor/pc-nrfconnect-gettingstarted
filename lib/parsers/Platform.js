...
export default class Platform {
    ...
    constructor(json) {
        try {
            this.platforms = Platform.extractPlatforms(json).map(platform => ({
            ...
            this.enabled = this.platforms
                .map(p => {
                    const { platform, osRelease, arch } = p;

                    if (!availablePlatforms.includes(platform)) {
                        throw new Error(
                            `"${platform}" is not valid for the "platform" field.`
                        );
                    }

                    if (!abailableArchs.includes(arch)) {
                        throw new Error(
                            `"${arch}" is not valid for the "arch" field.`
                        );
                    }
    ...
    static fromObject(obj) {
        if (!obj.platform) {
            throw new Error(
                `${obj} does not contain the mandatory property "platform".`
            );
        }
        return { platform: 'all', osRelease: 'any', arch: 'any', ...obj };
    }
    ...
    /**
     * @returns {Object} A JSON representation of the current instance.
     */
    asJSON() {
