...
export default class Checkable extends AbstractParser {
    ...
    constructor(json, id) {
        ...
        this.parseVariablesFrom(this._checkers);
    }
    ...
    get isManual() {
        ...
        return !this._checkers.some(item => item.enabled);
    }
    ...
    runCheckers() {
        ...
        return Promise.all(this._checkers.map(checker => Promise.all(checker.run())));
    }
}
