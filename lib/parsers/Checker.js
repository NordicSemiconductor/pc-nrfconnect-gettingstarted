    ...
    run() {
        ...
        return new Promise((resolve, reject) => {
            ...
            child.on('close', exitCode => {
                if (exitCode !== 0) {
                    reject({ command: this._command, exitCode, outputs });
                } else {
                    resolve({ command: this._command, exitCode, outputs });
                }
            });
        });
    }
...
class FileExistsChecker extends AbstractChecker {
    ...
    run() {
        ...
        // expected here
        return this._filenames.map(filename => stat
            .then(
                stats => Promise.resolve({
                    command: `Check for file: ${filename}`,
                    exitCode: 0,
                    ouputs: {
                        error: [],
