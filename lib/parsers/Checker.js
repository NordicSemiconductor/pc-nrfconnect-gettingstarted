...
class CommandChecker extends AbstractChecker {
    constructor(json) {
        super(json);
        this.commands =
            this.commands instanceof Array ? this.commands : [this.commands];
    }
...
class FileExistsChecker extends AbstractChecker {
    constructor(json) {
        super(json);
        this.filenames =
            this.filenames instanceof Array ? this.filenames : [this.filenames];
    }
...
export default function checkerFactory(json, id) {
    ...
    throw new Error(
        `"checkerType" field in Checker JSON must be either "command" or "fileExists". Received: ${json.checkerType}`
    );
}
